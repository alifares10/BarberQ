  ...
  return (
    <ScrollView contentContainerStyle={styles.contentContainer} contentInsetAdjustmentBehavior="automatic">
      <Card>
        ...
        <Text color="$colorMuted" textAlign={rtlLayout.textAlign}>{t('shopOwner.profileDescription')}</Text>
      </Card>

      <Card>
        <Text fontWeight="700" textAlign={rtlLayout.textAlign}>{profile?.full_name ?? t('shopOwner.profileFallbackName')}</Text>
        <Text color="$colorMuted" textAlign={rtlLayout.textAlign}>{profile?.phone ?? t('shopOwner.profileFallbackPhone')}</Text>
      </Card>

      <LanguageSettingsCard />

      <Card>
        <Text fontWeight="700" textAlign={rtlLayout.textAlign}>{t('shopOwner.profileAccountTitle')}</Text>
        <Text color="$colorMuted" textAlign={rtlLayout.textAlign}>{t('shopOwner.profileAccountDescription')}</Text>
